...
import getStyle from './homeStyles'
...
export default function Home() {
  ...
  const getWeatherData = async () => {
    try  {
        setIsLoading(true); 
        const response = await fetch(`https://api.openweathermap.org/data/3.0/onecall?lat=-6.82349&lon=39.26951&appid=${appId}&units=${units}`)
      ...
      <LinearGradient colors={['#224496', '#3b5998', '#192f6a']} style={styles.linearGradient}>
         ...
         <View style={styles.locationBar}>
             <MaterialCommunityIcons name="map-marker" size={26} color="#FFB200"  style={styles.iconlocation}/>
              <Text style={styles.location}>Dar es salaam</Text>
         </View>
